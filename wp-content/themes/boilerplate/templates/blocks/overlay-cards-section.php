...
$overlay_cards_subtitle = get_sub_field('overlay_cards_subtitle');
$overlay_cards_header = get_sub_field('overlay_cards_header');
$overlay_cards_header_content = get_sub_field('overlay_cards_header_content');
$overlay_cards_header_CTA = get_sub_field('overlay_cards_header_cta');
$overlay_cards_image = get_sub_field('overlay_cards_image');
$overlay_cards = get_sub_field('overlay_cards');
?>

<section class="overlay-cards-section">

  <div class="container">

    <div class="overlay-cards-header grid">
      <div class="overlay-cards-header--top">
        <?php if (!empty($overlay_cards_subtitle)) : ?>
          <div class="overlay-cards-header__subtitle">
            <?php echo esc_html($overlay_cards_subtitle); ?>
          </div>
        <?php endif; ?>
        <?php if (!empty($overlay_cards_header)) : ?>
          <div class="title"><?php echo esc_html($overlay_cards_header); ?></div>
        <?php endif; ?>
      </div>
      <div class="overlay-cards-header--bottom">
        <?php if (!empty($overlay_cards_header_content)) : ?>
          <?php echo wp_kses_post($overlay_cards_header_content); ?>
        <?php endif; ?>
        <?php if (get_sub_field('overlay_cards_header_cta')):
          $cta = get_sub_field('overlay_cards_header_cta'); ?>
          <div class="overlay-cards-header-cta">
    ...
    <div class="overlay-cards-scroll-wrapper">
      <div class="cards-overlay" style="background-image: url('<?php echo esc_url($overlay_cards_image['url']); ?>');">
        <div class="cards-overlay--cards">
          <?php if (!empty($overlay_cards)) : ?>
            <?php foreach ($overlay_cards as $card) : ?>
              <div class="card-overlay">
                <div class="card-overlay--icon">
                  <?php if (!empty($card['overlay_card_image'])) : ?>
                    <img src="<?php echo esc_url($card['overlay_card_image']['url']); ?>" alt="<?php echo esc_attr($card['overlay_card_image']['alt']); ?>">
                  <?php endif; ?>
                </div>
                <div class="card-overlay--content">
                  <?php if (!empty($card['overlay_card_title'])) : ?>
                    <h5><?php echo esc_html($card['overlay_card_title']); ?></h5>
                  <?php endif; ?>
                  <?php if (!empty($card['overlay_card_content'])) : ?>
                    <p class="card-overlay--text">
                      <?php echo wp_kses_post($card['overlay_card_content']); ?>
                    </p>
                  <?php endif; ?>
                </div>
              </div>
